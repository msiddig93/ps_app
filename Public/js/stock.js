$(function(){
      ...
      $('#add-element').click(function(e) {
          ...
          LoadElementImg(0);
          $('#action').val("stock.add");
          $('#modal-add-element .title-top').html('<span class="content-title"> <i class="fa fa-dropbox"></i> الموردين <i class="fa fa-chevron-left"></i> <h3>إضافة مورد جديد</h3> </span>');
        ...
        success: function (data) {
            ...
            if(data == 1)
            {
                $.notify({
                    ...
                    message: '  بيانات المورد '+ $('#vend_name').val() + ' بنجــاح .'
                },{
                    // settings
                    type: "success",
                    allow_dismiss: false,
...
function EditElement(btn , e)
{
    ...
    $('#modal-add-element .title-top').html('<span class="content-title"> <i class="fa fa-dropbox"></i> الموردين <i class="fa fa-chevron-left"></i> <h3>تعديل بيانات المورد</h3> </span>');
	$('#add').modal('show',2000);
}

function LoadElementEdit(btn) {
    var obj ={
        ajax_action : 'stock.LoadElementEdit',
    ...
    $.post(
        ...
        function(data)
        {
            ...
            $('#vend_name').val(stock.vend_name);
            $('#address').val(stock.address);
            $('#email').val(stock.email);
            $('#phone').val(stock.phone);
            $('#idElementID').val(stock.id);
            $('#action').val("stock.edit");
            LoadElementImg(stock.id);
        },
        'html'
		...
		function(data)
		{
			...
			if(data == 1)
			{
                $.notify({
                    ...
                    message: '  بيانات المورد رقم '+ $(btn).attr('element_id') + ' بنجــاح .'
                },{
                    // settings
                    type: "success",
                    allow_dismiss: false,
