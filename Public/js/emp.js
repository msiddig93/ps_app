$(function(){
      ...
      $('#add-element').click(function(e) {
          ...
          LoadEmpImg(0);
          $('#action').val("emp.add");
          $('#modal-add-element .title-top').html('<span class="content-title"> <i class="fa fa-users"></i> الموظفين <i class="fa fa-chevron-left"></i> <h3>إضافة موظف جديد</h3> </span>');
        ...
        success: function (data) {
            ...
            if(data == 1)
            {
                $.notify({
                    ...
                    message: '  بيانات الموظف '+ $('#fullname').val() + ' بنجــاح .'
                },{
                    // settings
                    type: "success",
                    allow_dismiss: false,
...
function EditElement(btn , e)
{
    ...
    $('#modal-add-element .title-top').html('<span class="content-title"> <i class="fa fa-users"></i> الموظفين <i class="fa fa-chevron-left"></i> <h3>تعديل بيانات الموظف</h3> </span>');
	$('#add').modal('show',2000);
}

function LoadElementEdit(btn) {
    var obj ={
        ajax_action : 'emp.LoadElementEdit',
    ...
    $.post(
        ...
        function(data)
        {
            ...
            $('#phone').val(emp.PHONE);
            $('#addrss').val(emp.ADDRSS);
            $('#TYPE').val(emp.TYPE);
            $('#BRANSH_ID').val(emp.BRANSH_ID);
            $('#login').val(emp.LOGIN);
            $('#pass').val(emp.PASS);
            $('#idElementID').val(emp.ID);
            $('#action').val("emp.edit");
            LoadEmpImg(emp.ID);
        },
        'html'
...
function LoadEmpImg(id) {
    var obj ={
        ajax_action : 'emp.loadEmpImg',
        id : id
    };
		...
		function(data)
		{
			...
			if(data == 1)
			{
                $.notify({
                    ...
                    message: '  بيانات الموظف رقم '+ $(btn).attr('element_id') + ' بنجــاح .'
                },{
                    // settings
                    type: "success",
                    allow_dismiss: false,
