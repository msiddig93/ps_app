$(function(){
      ...
      $('#add-element').click(function(e) {
          ...
          // LoadElementImg(0);
          $('#action').val("category.add");
          $('#modal-add-element .title-top').html('<span class="content-title"> <i class="fa fa-users"></i> الاصناف <i class="fa fa-chevron-left"></i> <h3>إضافة صنف جديد</h3> </span>');
...
{
   var obj ={
      ajax_action : 'category.loadAddID'
   };
        ...
        success: function (data) {
            ...
            if(data == 1)
            {
                $.notify({
                    ...
                    message: '  بيانات الصنف '+ $('#cat_name').val() + ' بنجــاح .'
                },{
                    // settings
                    type: "success",
                    allow_dismiss: false,
...
function EditElement(btn , e)
{
    ...
    $('#modal-add-element .title-top').html('<span class="content-title"> <i class="fa fa-users"></i> الاصناف <i class="fa fa-chevron-left"></i> <h3>تعديل بيانات الصنف</h3> </span>');
	$('#add').modal('show',2000);
}

function LoadElementEdit(btn) {
    var obj ={
        ajax_action : 'category.LoadElementEdit',
    ...
    $.post(
        ...
        function(data)
        {
            ...
            $('#cat_desc').val(category.cat_desc);
            $('#idElementID').val(category.id);
            $('#action').val("category.edit");
            // LoadElementImg(category.id);
        },
        'html'
...
function LoadElementImg(id) {
    var obj ={
        ajax_action : 'category.loadEmpImg',
        id : id
    };
		...
		function(data)
		{
			...
			if(data == 1)
			{
                $.notify({
                    ...
                    message: '  بيانات الصنف رقم '+ $(btn).attr('element_id') + ' بنجــاح .'
                },{
                    // settings
                    type: "success",
                    allow_dismiss: false,
